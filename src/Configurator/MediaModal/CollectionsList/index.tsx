...
export default function CollectionsList(props: Props) {
	...
	const collectionEditClickHandler = React.useCallback(
		(collectionId) => {
	...
	const collectionDeleteClickHandler = React.useCallback(
		(collectionID) => {
	...
	const collectionsElements = !props.media?.collections
		? null
		: Object.entries(props.media?.collections).map(([collID, collection], index) => {
				...
				if (resolutionsIDs.length) {
					...
					previewContent = <>{clippers}</>;
				} else {
					previewContent = (
						<>
							<div>
								<span className="fade-blink" title="Edit this collection to see preview here">
									Empty Slot
								</span>
							</div>
						</>
				...
				return (
					<div className="collection" key={`${collection.name}-collection${index}`}>
						<div
							...
							onClick={() => resolutionsIDs.length && props.useSelectedCollection(collID)}
						>
							{previewContent}
							<div className={collectionsClassName} onClick={(e) => void e.stopPropagation()}>
								...
								<DeleteIcon
									id="delete-coll"
									onClick={(e) => collectionDeleteClickHandler(collID)}
								/>
							</div>
						</div>
