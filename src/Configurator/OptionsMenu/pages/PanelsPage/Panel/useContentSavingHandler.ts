...
export default function useContentSavingHandler<T>(
	onValueChange: (name: string, content: T) => void,
	panelName: string,
	initialContent?: T
): ContentSavingHandler<T> {
	const [content, setContent] = useState<T>(initialContent || null);

	const onContentChangedHandlerRef = useCallback(
		(newContent?: T) => {
